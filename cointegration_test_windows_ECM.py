...
# -*- coding: utf-8 -*-
"""
高级版协整分析+交易流程完整测试文件（滚动窗口版本+参数优化+ECM误差修正模型）
使用滚动窗口进行协整检验：
...
ECM误差修正模型功能：
1. 使用误差修正模型（ECM）预测价差的均值回归
2. 基于协整关系的长期均衡和短期偏离
3. 使用ECM模型计算Z-score，提高交易信号的准确性
4. 支持传统方法、ARIMA-GARCH和ECM三种策略
"""
...
# 导入Z-score策略
try:
    from strategies import TraditionalZScoreStrategy, ArimaGarchZScoreStrategy, EcmZScoreStrategy, BaseZScoreStrategy
...
class AdvancedCointegrationTrading:
    ...
    def backtest_cointegration_trading(self, data, selected_pairs, initial_capital=10000):
        """回测协整交易策略（使用原序列价差，支持传统方法、ARIMA-GARCH和ECM策略）"""
...
def test_rolling_window_cointegration_trading(csv_file_path):
    """
    滚动窗口协整分析+交易测试（支持传统方法、ARIMA-GARCH和ECM策略）

    Args:
        csv_file_path: CSV文件路径
    """
    print("=" * 80)
    print("滚动窗口协整分析+交易测试（支持传统方法、ARIMA-GARCH和ECM策略）")
    print("=" * 80)
...
class ParameterOptimizer:
    """
    参数优化器类（支持传统方法、ARIMA-GARCH和ECM策略）
    支持三种优化方法：
    1. 网格搜索（粗粒度 + 细粒度分层搜索）
    2. 贝叶斯优化
    3. 随机搜索
...
def test_parameter_optimization(csv_file_path):
    """
    参数优化测试函数（支持传统方法、ARIMA-GARCH和ECM策略）
    """
    print("=" * 80)
    print("参数优化测试（支持传统方法、ARIMA-GARCH和ECM策略）")
    print("=" * 80)
...
def main():
    ...
    主函数
    """
    print("滚动窗口协整分析+交易流程完整测试（带参数优化+ECM误差修正模型）")
    print("使用滚动窗口进行协整检验：")
