...
# -*- coding: utf-8 -*-
"""
高级版协整分析+交易流程完整测试文件（修正版）
使用正确的协整检验逻辑：
1. 先检验原序列的平稳性
2. 只有当两个原序列都是I(1)时，才进行协整检验
3. 检验原序列价差的平稳性（不是差分的价差）
"""
...
def enhanced_cointegration_test(price1, price2, symbol1, symbol2):
    """
        ...
        for j in range(i+1, len(symbols)):
            ...
            try:
                ...
                if coint_result['cointegration_found']:
                    ...
                    print(f"\n✓ {symbol1}/{symbol2} 协整检验通过")
                else:
                    print(f"\n✗ {symbol1}/{symbol2} 协整检验未通过")
...
def display_candidates_for_selection(candidates):
    ...
    for i, candidate in enumerate(candidates, 1):
        ...
        adf_p = candidate['spread_adf']['p_value']
        price1_order = candidate['price1_order']
        price2_order = candidate['price2_order']
...
def test_advanced_cointegration_trading(csv_file_path):
    """
    高级版协整分析+交易测试（使用正确的协整检验逻辑）
    
    Args:
        csv_file_path: CSV文件路径
    """
    print("=" * 80)
    print("高级版协整分析+交易测试（修正版）")
    print("=" * 80)
        ...
        if results['trades']:
            ...
            for pair, trades in pair_trades.items():
                ...
                for trade in trades:
                    ...
                    print(f"  {trade['timestamp']}: {action} {trade['symbol1']}={trade['symbol1_price']:.2f}, {trade['symbol2']}={trade['symbol2_price']:.2f}{pnl_info}")
        else:
            print("本次测试无交易记录")
