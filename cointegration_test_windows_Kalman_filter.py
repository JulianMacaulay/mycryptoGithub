...
# -*- coding: utf-8 -*-
"""
高级版协整分析+交易流程完整测试文件（滚动窗口版本+参数优化+Kalman Filter动态价差模型）
使用滚动窗口进行协整检验：
...
Kalman Filter动态价差模型功能：
1. 使用Kalman Filter动态估计价差的均值和方差
2. 基于状态空间模型，实时更新价差的估计值
3. 使用Kalman Filter估计计算Z-score，提高交易信号的准确性
4. 支持传统方法、ARIMA-GARCH、ECM和Kalman Filter四种策略
"""
...
# 导入Z-score策略
try:
    from strategies import TraditionalZScoreStrategy, ArimaGarchZScoreStrategy, EcmZScoreStrategy, KalmanFilterZScoreStrategy, BaseZScoreStrategy
...
class AdvancedCointegrationTrading:
    ...
    def backtest_cointegration_trading(self, data, selected_pairs, initial_capital=10000):
        """回测协整交易策略（使用原序列价差，支持传统方法、ARIMA-GARCH、ECM和Kalman Filter策略）"""
...
def test_rolling_window_cointegration_trading(csv_file_path):
    """
    滚动窗口协整分析+交易测试（支持传统方法、ARIMA-GARCH、ECM和Kalman Filter策略）

    Args:
        csv_file_path: CSV文件路径
    """
    print("=" * 80)
    print("滚动窗口协整分析+交易测试（支持传统方法、ARIMA-GARCH、ECM和Kalman Filter策略）")
    print("=" * 80)
...
class ParameterOptimizer:
    """
    参数优化器类（支持传统方法、ARIMA-GARCH、ECM和Kalman Filter策略）
    支持三种优化方法：
    1. 网格搜索（粗粒度 + 细粒度分层搜索）
    2. 贝叶斯优化
    3. 随机搜索
...
def test_parameter_optimization(csv_file_path):
    """
    参数优化测试函数（支持传统方法、ARIMA-GARCH、ECM和Kalman Filter策略）
    """
    print("=" * 80)
    print("参数优化测试（支持传统方法、ARIMA-GARCH、ECM和Kalman Filter策略）")
    print("=" * 80)
