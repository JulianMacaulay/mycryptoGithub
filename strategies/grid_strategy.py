...
在价格区间内设置多个买卖网格，价格下跌时买入，价格上涨时卖出
"""
                    ...
                    if current_idx > 0:
                        ...
                        if prev_price > grid_level:  # 从上方跌下来
                            signals.append({
                                'signal': 'long',
                                'price': grid_level,
                                'reason': f'网格买入（网格{i+1}，价位{grid_level:.4f}）',
                                'grid_level': grid_level,
                                'grid_index': i,
                                'position_id': position_id,
                                '_grid_strategy': True  # 标记这是网格策略信号
                            })
                ...
                if i < len(self.grid_levels) - 1:
                    ...
                    if high_price >= upper_grid:
                        signals.append({
                            'signal': 'close_long',
                            'price': upper_grid,
                            'reason': f'网格止盈（网格{i+1}，价位{upper_grid:.4f}）',
                            'grid_level': grid_level,
                            'grid_index': i,
                            'position_id': position_id,
                            '_grid_strategy': True  # 标记这是网格策略信号
                    ...
                    if high_price >= take_profit_price:
                        signals.append({
                            ...
                            'reason': f'网格止盈（网格{i+1}，盈利{self.take_profit_pct*100:.1f}%）',
                            'grid_level': grid_level,
                            'grid_index': i,
                            'position_id': position_id,
                            '_grid_strategy': True  # 标记这是网格策略信号
                ...
                if i > 0:
                    ...
                    if low_price <= lower_grid:
                        signals.append({
                            'signal': 'close_long',
                            'price': lower_grid,
                            'reason': f'网格止损（网格{i+1}，价位{lower_grid:.4f}）',
                            'grid_level': grid_level,
                            'grid_index': i,
                            'position_id': position_id,
                            '_grid_strategy': True  # 标记这是网格策略信号
                    ...
                    if low_price <= stop_loss_price:
                        signals.append({
                            ...
                            'reason': f'网格止损（网格{i+1}，亏损{self.stop_loss_pct*100:.1f}%）',
                            'grid_level': grid_level,
                            'grid_index': i,
                            'position_id': position_id,
                            '_grid_strategy': True  # 标记这是网格策略信号
