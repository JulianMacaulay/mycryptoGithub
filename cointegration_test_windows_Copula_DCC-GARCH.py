...
# -*- coding: utf-8 -*-
"""
高级版协整分析+交易流程完整测试文件（滚动窗口版本+参数优化+Copula + DCC-GARCH相关性/波动率模型）
使用滚动窗口进行协整检验：
...
Copula + DCC-GARCH相关性/波动率模型功能：
1. 使用DCC-GARCH模型估计动态波动率和动态相关性
2. 使用Copula建模两个资产之间的依赖结构
3. 基于动态波动率和相关性计算价差的动态方差
4. 使用Copula + DCC-GARCH估计计算Z-score，提高交易信号的准确性
5. 支持传统方法、ARIMA-GARCH、ECM、Kalman Filter和Copula + DCC-GARCH五种策略
"""
...
# 导入Z-score策略
try:
    from strategies import TraditionalZScoreStrategy, ArimaGarchZScoreStrategy, EcmZScoreStrategy, KalmanFilterZScoreStrategy, CopulaDccGarchZScoreStrategy, BaseZScoreStrategy
